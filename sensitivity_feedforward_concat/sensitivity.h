...
int sensitivity(const vector<double> &geny, const char* fnm)
{
    ...
    // creo la matriz que va a tener los resultados
    ...
    for(int index=0; index<param_len; ++index)
    {
        ...
        while(variation > 0.001)
        {
            ...
            new_param_set = geny;
            //cout << new_param_set[0] << " " << new_param_set[11] << endl;
            perturbation += variation;
            new_param_set[index] *= pow(10, perturbation);
            //cout << new_param_set[index] << endl;
            ...
            variation /= 2;
            //cout << variation << " " << perturbation << " "<< log10(new_param_set[index]/geny[index]) << endl;
